...
resource "null_resource" "setup_installer" {
  ...
  count = "${var.enable_ha == "false" && var.enable_bluemix_install == "true" ? length(var.vm_ipv4_address_list) : 0}"
...
resource "null_resource" "setup_installer_tar" {
  ...
  count = "${var.enable_ha == "false" && var.enable_bluemix_install == "false" ? length(var.vm_ipv4_address_list) : 0}"
