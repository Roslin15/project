resource "null_resource" "config_icp_HA_boot_dependsOn" {
...
resource "null_resource" "setup_HA_installer" {
  depends_on = ["null_resource.config_icp_HA_boot_dependsOn"]
  count = "${var.enable_ha == "true" ? length(var.vm_ipv4_address_list) : 0}"
...
resource "null_resource" "run_icp_HA_installer" {
  depends_on = ["null_resource.setup_HA_installer"]
  count = "${var.enable_ha == "true" ? length(var.vm_ipv4_address_list) : 0}"
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "docker run -e LICENSE=accept --net=host --rm -v /usr/local/bin:/data ibmcom/${var.kub_version}-ee cp /kubectl /data",
    ]
  }
}

resource "null_resource" "icp_install_HA_finished" {
  depends_on = ["null_resource.run_icp_HA_installer","null_resource.config_icp_HA_boot_dependsOn","null_resource.setup_HA_installer"]
